...
class SelectSearch extends React.Component {
  ...
  constructor(props) {
    ...
    this.classes = {
      container: this.props.multiple
        ? `${this.props.className} ${Bem.m(this.props.className, 'multiple')}`
      ...
      focus: this.props.multiple
        ? `${this.props.className} ${Bem.m(
            this.props.className,
            'multiple focus'
          )}`
        : `${this.props.className} ${Bem.m(this.props.className, 'focus')}`
    };
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      this.state.options.length === nextState.defaultOptions.length ||
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.options !== this.state.defaultOptions) {
      ...
      nextState.options = this.getNewOptionsList(
        flattenedOptions,
        this.state.search
      );
  ...
  onFocus = event => {
    this.setState({ focus: true, search: this.state.search || '' });
  };
  ...
  removeOption(value) {
    ...
    setTimeout(() => {
      this.props.onChange.call(
        null,
        this.publishOption(optionValue),
        this.state,
        this.props
      );
    }, 50);
  ...
  scrollToSelected(force = false, selected = 'hover') {
    ...
    const selectedItem = this.selectOptions.current.querySelector(
      `.${Bem.m(this.classes.option, selected)}`
    );
    ...
    if (foundOptions && foundOptions.length > 0) {
      ...
      if (groupedOptions && groupedOptions.length) {
        groupedOptions.forEach(option => {
          if (
            {}.hasOwnProperty.call(option, 'type') &&
            option.type === 'group'
          ) {
            const subOptions = [];
            ...
            option.items.forEach(groupOption => {
              subOptions.push(
                this.renderOption(groupOption, stateValue, multiple)
              );
            });
            ...
            options.push(
              <li className={this.classes.row} key={option.groupId}>
                <div className={this.classes.group}>
                  ...
                  <ul className={this.classes.options}>{subOptions}</ul>
                </div>
              </li>
            );
          } else {
  ...
  renderSearchField() {
    ...
    if (this.props.search) {
      ...
      if (!this.state.value) {
        ...
        labelClassName = `${this.classes.search} ${Bem.m(
          this.classes.search,
          'placeholder'
        )}`;
      } else {
  ...
  render() {
    const className = this.state.focus
      ? this.classes.focus
